...
Foam::myPhiScheme<Type, myPhiLimiter>::limiter
(
...
) const
{
    ...
    if (this->faceFlux_.dimensions() == dimDensity*dimVelocity*dimArea*dimLength)
    {
    ...
    else if (this->faceFlux_.dimensions() != dimVelocity*dimArea*dimLength)
    {
    ...
    {
        pLimiter[face] = myPhiLimiter::limiter
        (
            CDweights[face],
            Uflux[face],
            phi[owner[face]],
